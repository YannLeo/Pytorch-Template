...
class WiSigDiffRecvDataset(Dataset):
    ...
    """
    def __init__(self, path='/home/rxy/Datasets/WiSig/DiffRecv', Rx: list[int] = None, train=True):
        super().__init__()
        data, targets = [], []
        for r in Rx:
...
class WiSigDiffRecvDatasetUneven(Dataset):
    """
    Rx: part of ["1-1", "1-19", "2-1", "2-19", "3-19", "7-7", "7-14", "8-8", "14-7", "18-2", "19-2", "20-1"]
    Randomly discard half of the data of some classes to make the dataset unbalanced.
    """
    def __init__(self, path='/home/rxy/Datasets/WiSig/DiffRecv', Rx: list[int] = None, discarded_num: int = 1, train=True):
        super().__init__()
        data, targets = [], []
        discard_labels = ([], [3], [2, 3], [1, 2, 3], [1, 2, 3, 5])[discarded_num]  # 0~5
        for r in Rx:
            dataset = np.load(Path(path) / r / ('train.npz' if train else 'test.npz'))
            idx = []
            for label in discard_labels:
                _idx = np.where(dataset['targets'] == label)[0]
                _idx = _idx[_idx.shape[0] // 2:]
                idx.append(_idx)  # delete half of the data            
            
            _data = np.delete(dataset['data'], np.concatenate(idx, 0), axis=0)
            _targets = np.delete(dataset['targets'], np.concatenate(idx, 0), axis=0)
        ...
        print(f"Successfully loaded dataset on Rx of {Rx}, with shape of {self.data.shape}, {self.targets.shape}"
              f" and discarded labels of {discard_labels}.")
